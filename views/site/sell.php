...
?>
<?php 
    $url = \yii\helpers\Url::to(['product/fetch']);
    $detail = \yii\helpers\Url::to(['product/detail']);
...
function pro(item){
    ...
    $.ajax({
         url: "<?=$details?>?product_id=" + product_id ,
         dataType: 'json',
         method: 'GET',
         data: product_id,
...
<div class="invoices-form" style="margin-top: 30px;">
    ...
    <table class="table table-borderd table-responsive container-items">
        <tr class="">
            <th style="width: 20%;"><?=Yii::t('app', 'Inventory')?></th>
            <th style="width: 20%;"><?=Yii::t('app', 'Item')?></th>
            <th style="width: 11%;"><?=Yii::t('app', 'Available')?></th>
            <th style="width: 11%;"><?=Yii::t('app', 'Quantity')?></th>
            <th style="width: 13%;"><?=Yii::t('app', 'Price')?></th>
            <th style="width: 8%;"><?=Yii::t('app', 'Discount')?></th>
            <th style="width: 13%;"><?=Yii::t('app', 'LineTotal')?></th>
            <th style="width: 4%;">
        ...
        <tr class="item">
            ...
            ?>
            <td style="width: 20%;">
                <?= $form->field($modelItem, "[{$i}]inventory")->dropDownList(
                ...
                <?= $form->field($modelItem, "[{$i}]available")
                    ->textInput(
                        [
                            ...
                            'onchange' => 'check($(this))',
                            'readonly' => true, 
                            'placeholder'=>Yii::t('app', 'Available'),
                        ])
                    ->label(false) 
                ?>
            </td>
            ...
            <td style="width: 8%;">
                <?= $form->field($modelItem, "[{$i}]discount")
                    ->textInput(
                        [
                            'type' => 'number',
                            'placeholder'=>Yii::t('app', 'Discount'),
                            'disabled' => true,
                            // 'onchange' => 'check($(this))',
                        ])
                    ->label(false) 
    ...
    <div class="form-group">
        <?= Html::submitButton(Yii::t('app', 'Create'), ['class' => " btn btn-flat btn-block"]) ?>
    </div>
